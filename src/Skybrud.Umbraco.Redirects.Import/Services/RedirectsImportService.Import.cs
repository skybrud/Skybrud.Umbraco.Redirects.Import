...
using Skybrud.Umbraco.Redirects.Import.Helpers;
...
public partial class RedirectsImportService {
    ...
    public virtual ImportResult Import(IImportOptions options, DataTable dataTable) {
        ...
        RedirectsImportHelper helper = new(_dependencies, dataTable, options);
    ...
    public virtual void Import(RedirectImportItem item) {
        ...
        if (string.IsNullOrWhiteSpace(item.Options.OriginalUrl)) {
        ...
        try {
            ...
            Guid rootNodeKey = item.Options.RootNodeKey ?? Guid.Empty;

            IRedirect? existing = _redirectsService.GetRedirectByUrl(rootNodeKey, item.Options.OriginalUrl);

            if (!item.Options.Overwrite && existing != null) {
            ...
            AddRedirectOptions options = new() {
                Overwrite = item.Options.Overwrite,
                RootNodeKey = rootNodeKey,
                OriginalUrl = item.Options.OriginalUrl ?? throw new WtfException(),
                Destination = item.Options.Destination ?? throw new WtfException(),
                Type = item.Options.Type ?? throw new WtfException(),
                ForwardQueryString = item.Options.ForwardQueryString ?? throw new WtfException(),
            };

            if (existing == null) {
                _redirectsService.AddRedirect(options);
                item.Status = RedirectImportStatus.Added;
                return;
            }

            if (HasChanges(existing, options)) {
                existing.Destination = item.Options.Destination;
                existing.Type = options.Type;
                existing.ForwardQueryString = options.ForwardQueryString;
