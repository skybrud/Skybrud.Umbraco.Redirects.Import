...
public partial class RedirectsImportService {
    ...
    public IEnumerable<IRedirect> GetRedirects(IExportOptions options) {
        return _redirectsService.GetRedirects();
    }
    ...
    public virtual DataTable ExportAsDataTable(IExportOptions options) {
        ...
        Dictionary<string, Func<IRedirect, string>> hej = new() {
            ...
            {"QueryString", x => x.QueryString ?? string.Empty},
            ...
            {"DestinationQuery", x => x.Destination.Query ?? string.Empty},
            {"DestinationFragment", x => x.Destination.Fragment ?? string.Empty},
            {"DestinationName", x => x.Destination.Name ?? string.Empty},
        ...
        foreach (IRedirect redirect in GetRedirects(options)) {
            ...
            foreach (ExportColumnItem column in columns) {
                if (!column.IsSelected) continue;
                if (!hej.TryGetValue(column.Alias, out var callback)) continue;
                row[column.Alias] = callback(redirect);
            }
        }
    ...
    public virtual string GetContentType(string? extension) {
        return extension?.Trim('.') switch {
            "csv" => RedirectsImportConstants.ContentTypes.Csv,
            "json" => RedirectsImportConstants.ContentTypes.Json,
            "xlsx" => RedirectsImportConstants.ContentTypes.Xlsx,
    ...
    /// <summary>
    /// Reads the request body and parses it as a <see cref="JObject"/>.
    /// </summary>
    /// <param name="request">The HTTP request.</param>
    /// <returns>The parsed <see cref="JObject"/>, or <c>null</c> if parsing fails.</returns>
    public async Task<JObject?> GetJsonBodyAsync(HttpRequest request) {
